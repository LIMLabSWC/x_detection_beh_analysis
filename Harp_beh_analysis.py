...
pkl2use = os.path.join(pkldir,'mouse_hf_fam2_2d_90Hz_lpass4_hpass00_hanning025_TOM_w_LR_detrend_wTTL_nospdouts.pkl')
run = Main(pkl2use, (-1.0, 3.0), figdir=rf'W:\mouse_pupillometry\figures\mouse_fam_13start_hpass0',fig_ow=False)
...
harpmatrices_pkl = os.path.join(pkldir, 'mousefam_hf_harps_matrices_allfam.pkl')
harp_pkl_ow_flag = False
if os.path.isfile(harpmatrices_pkl) and not harp_pkl_ow_flag:
...
events2align = ['Pretone_end','Gap_Time']
...
harp_aligned_pkl = r'pickles\fam_harp_aligned.pkl'
if os.path.isfile(harp_aligned_pkl):
...
for ei, event2align in enumerate(events2align):
    for outcome, align_col in zip([['e!0', ], ['e=0']],['ToneTime','Pretone_end']):
        run.animals = run.labels
        if f'{outcome[0]}_{event2align}' not in run.lickrasters_firstlick.keys() or redo_align:
            run.lickrasters_firstlick[f'{outcome[0]}_{event2align}'] = run.get_aligned_events(run, f'{event2align}_dt', 0, (-1.0, 3.0),
...
lick_ts_fig.show()
lick_ts_fig.savefig(os.path.join(run.figdir, f'allsess_HF_lickrate_ts_{"_".join(events2align)}.svg'), bbox_inches='tight')
...
for ci, cond in enumerate([['e!0', ], ['e=0']]):
    ...
    for sess in run.data:
        mean_perf = utils.filter_df(run.data[sess].trialData,cond)['Trial_Outcome'].mean()
...
for sess in run.data:
    ...
    for i, ntone_list in enumerate(ntrones_rt_list):
        ntones_df = utils.filter_df(sess_td,[f'tones{i+1}'])
